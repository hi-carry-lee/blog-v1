...
export default function CategoryForm({
   ...
   * 🔄 表单数据同步 Effect
   *
   * 作用：当 dialog 打开/关闭或 category 变化时，同步表单数据
   *
   * 两种情况：
   * 1. category 存在（编辑模式）：填充现有数据
   * 2. category 为 null（创建模式）：清空表单
   *
   ...
   * - 这解决了：创建成功后再次打开时，category 仍为 null 导致 useEffect 不触发的问题
  ...
  const handleSlugBlur = async () => {
    ...
    if (!slug || (isEditMode && slug === category?.slug)) {
      return;
    }
  ...
  const onSubmit = async (data: CategoryFormData) => {
    try {
      ...
      logger.error("Category form submission error", err);
